...
**Design tokens:** All colors, fonts, and spacing are CSS custom properties in `src/styles/tokens.css`. Two fonts only: `--font-serif` (Playfair Display, for headings) and `--font-sans` (Inter, for everything else). Never add `font-family` declarations in components — use the tokens.
...
**Scoped styles:** Each section component uses Astro's scoped `<style>` block. Dark-background sections (Gallery uses `--color-ink`, Rules uses `--color-sea`) override `.section-label` and `.section-title` colors with direct class selectors inside the component — not `:global()`.
